...
export function QuestionSummary({ question, notation }: { question: PracticeQuestion; notation: ChordNotation }) {
  ...
  if (question.kind === 'spread-triad-fill') return <><TriadName question={question} /> · Spread {question.pattern}</>
  ...
  return <><PitchName value={question.key.tonic} /> 大调 · {question.chords.map((chord) => chord.roman).join('–')} · {question.voicingMode}</>
}
