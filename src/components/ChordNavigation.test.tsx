...
describe('chord navigation', () => {
  it('uses the requested family and triad hierarchy', () => {
    ...
    expect(screen.getByRole('button', { name: /Closed Triad/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Spread Triad/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /三音·五音/ })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /Spread Triad/ }))
  ...
  it('shows Shell and Drop 2 cards without a seventh-chord corner badge', () => {
    ...
    const shell = screen.getByRole('button', { name: /Shell Voicing/ })
    const drop2 = screen.getByRole('button', { name: /Drop 2 Voicing/ })
    ...
    const icon = within(drop2).getByRole('img', { name: 'Drop 2：第二高音 D 下移八度到低音 D' })
  ...
  it('keeps each triad setup independent while sharing quality and spelling controls', () => {
    ...
    expect(screen.getByText('设置 Spread Triad')).toBeInTheDocument()
    expect(screen.getByText('和弦性质')).toBeInTheDocument()
    expect(screen.getByText('和弦音名范围')).toBeInTheDocument()
    expect(screen.queryByText('成员排列')).not.toBeInTheDocument()

    rerender(<ChordSetup {...props} kind="chord-tone" />)
    expect(screen.getByText(/固定播放 Closed 原位/)).toBeInTheDocument()
    expect(screen.queryByText('Closed Triad', { selector: 'button' })).not.toBeInTheDocument()
