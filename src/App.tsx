...
type View = 'home' | 'chord' | 'key' | 'setup-interval' | 'setup-triad' | 'setup-key' | 'quiz' | 'summary' | 'wrongs'
...
function App() {
  ...
  const [progressionQualities, setProgressionQualities] = useState<TriadQuality[]>([])
  ...
  useEffect(() => {
    ...
    if (question.kind === 'triad-fill') setNoteValues(['', '', ''])
      ...
      setProgressionQualities(question.direction === 'forward' ? ['major', 'major', 'major', 'major'] : [])
    } else {
      setNoteValues([])
      setDegreeValues([])
    }
  ...
  const submitNotes = () => {
    ...
    if (question.kind === 'progression' && question.direction === 'forward') {
      const rootsCorrect = question.triads.every((triad, index) => noteValues[index] === pitchName(triad.root))
      const qualitiesCorrect = question.triads.every((triad, index) => progressionQualities[index] === triad.quality)
      const enharmonic = !rootsCorrect && question.triads.some((triad, index) => noteValues[index] && pitchClassIsEnharmonic(noteValues[index], pitchName(triad.root)))
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        {view === 'home' && <HomeView stats={stats} installPrompt={installPrompt} onInstall={() => void triggerInstall()} onChoose={(next) => { setKind(next); setView(next === 'interval' ? 'setup-interval' : next === 'triad-fill' ? 'chord' : 'key') }} />}

        {view === 'chord' && <section className="panel navigation-panel">
          <button type="button" className="back-button" onClick={() => setView('home')}>← 返回首页</button>
          <div className="eyebrow">CHORDS</div><h1>和弦模块</h1><p className="setup-copy">三和弦已就绪；未来七和弦会在这里与它并列。</p>
          <button type="button" className="submodule-card" onClick={() => { setKind('triad-fill'); setView('setup-triad') }}><span>01</span><strong>三和弦</strong><small>转位音名填空、三音 / 五音问答</small></button>
        </section>}
        ...
        {view === 'setup-triad' && <TriadSetup kind={kind} settings={settings} setSettings={setSettings} starting={starting} notice={notice} onBack={() => setView('chord')} onKindChange={setKind} onStart={() => void beginSession(kind)} />}
        ...
        {view === 'quiz' && question && <section className="quiz-view">
          ...
          <article className={`question-card ${feedback ? feedback.correct ? 'feedback-correct' : 'feedback-wrong' : ''}`}>
            ...
            {question.kind === 'triad-fill' && <TriadFillExercise question={question} feedback={feedback} noteValues={noteValues} activeSlot={activeSlot} onActiveSlotChange={setActiveSlot} onChange={setNoteValues} onPlay={() => void playQuestion()} onSubmit={submitNotes} onValuePlay={(value, index) => void playFilledNote(value, index)} />}
            {question.kind === 'chord-tone' && <ChordToneExercise question={question} feedback={feedback} noteValues={noteValues} onChange={setNoteValues} onPlay={() => void playQuestion()} onSubmit={submitNotes} />}
        ...
        {view === 'wrongs' && <section className="wrong-view panel">
          ...
          <div className="chip-row">{(['all', 'interval', 'triad-fill', 'chord-tone', 'scale-degree', 'progression'] as const).map((filter) => <button type="button" key={filter} className={wrongFilter === filter ? 'chip selected' : 'chip'} onClick={() => setWrongFilter(filter)}>{filter === 'all' ? '全部' : KIND_NAMES[filter]}</button>)}</div>
          {filteredWrongItems.length ? <><div className="wrong-list">{filteredWrongItems.map((item) => <div className="wrong-row" key={item.key}><span className="wrong-kind">{KIND_NAMES[item.kind]}</span><strong>{questionSummary(item.question)}</strong><small>累计答错 {item.wrongCount} 次</small></div>)}</div><button type="button" className="primary-button" disabled={starting} onClick={() => void beginSession(filteredWrongItems[0].kind, filteredWrongItems.slice(0, 10).map((item) => item.question))}>{starting ? '正在准备音源…' : `复习这 ${Math.min(10, filteredWrongItems.length)} 题`}</button></> : <div className="empty-state"><span>✓</span><h2>这里暂时没有错题</h2><p>答错的题会按练习类型保存在这里。</p></div>}
        </section>}
      </main>
...
function HomeView({ stats, installPrompt, onInstall, onChoose }: { stats: LifetimeStats; installPrompt: BeforeInstallPromptEvent | null; onInstall: () => void; onChoose: (kind: 'interval' | 'triad-fill' | 'scale-degree') => void }) {
  return <section className="home-view">
    ...
    <div className="mode-grid three">
      ...
      <button type="button" className="mode-card triad-mode" onClick={() => onChoose('triad-fill')}><span className="mode-number">02</span><span className="mode-icon">△</span><h2>和弦</h2><p>目前包含三和弦的转位填空与三音、五音问答。</p><span className="mode-link">进入和弦 →</span></button>
...
function TriadSetup({ kind, settings, setSettings, starting, notice, onBack, onKindChange, onStart }: { kind: PracticeKind; settings: AppSettings; setSettings: React.Dispatch<React.SetStateAction<AppSettings>>; starting: boolean; notice: string; onBack: () => void; onKindChange: (kind: 'triad-fill' | 'chord-tone') => void; onStart: () => void }) {
  return <section className="setup-view panel"><button type="button" className="back-button" onClick={onBack}>← 返回和弦模块</button><div className="eyebrow">TRIADS</div><h1>设置三和弦练习</h1>
    ...
    <button type="button" className="primary-button" disabled={!settings.triad.qualities.length || starting} onClick={onStart}>{starting ? '正在准备钢琴音源…' : '开始 10 题练习'}</button>{notice && <p className="notice" role="alert">{notice}</p>}
  </section>
}
...
function KeySetup({ kind, settings, setSettings, starting, notice, onBack, onStart }: { kind: PracticeKind; settings: AppSettings; setSettings: React.Dispatch<React.SetStateAction<AppSettings>>; starting: boolean; notice: string; onBack: () => void; onStart: () => void }) {
  ...
  return <section className="setup-view panel"><button type="button" className="back-button" onClick={onBack}>← 返回调模块</button><div className="eyebrow">KEY TRAINING</div><h1>{isScale ? '设置音名与音级' : '设置和弦进行与级数'}</h1>
    ...
    {isScale ? <p className="setup-copy">十题保证 1–7 级各出现一次，另外三题复习；顺序每轮不同。</p> : <><fieldset><legend>进行声部</legend><div className="segmented"><button type="button" className={settings.keyPractice.voiceCount === 3 ? 'selected' : ''} onClick={() => setSettings((current) => ({ ...current, keyPractice: { ...current.keyPractice, voiceCount: 3 } }))}>三声部</button><button type="button" className={settings.keyPractice.voiceCount === 4 ? 'selected' : ''} onClick={() => setSettings((current) => ({ ...current, keyPractice: { ...current.keyPractice, voiceCount: 4 } }))}>四声部</button></div></fieldset><p className="setup-copy">十题覆盖九条常见进行模板，第十题随机复习其中一条。</p></>}
...
function TriadFillExercise({ question, feedback, noteValues, activeSlot, onActiveSlotChange, onChange, onPlay, onSubmit, onValuePlay }: { question: Extract<PracticeQuestion, { kind: 'triad-fill' }>; feedback: AnswerFeedback | null; noteValues: string[]; activeSlot: number; onActiveSlotChange: (index: number) => void; onChange: (values: string[]) => void; onPlay: () => void; onSubmit: () => void; onValuePlay: (value: string, index: number) => void }) {
  return (
    <>
      <QuestionHeading eyebrow="INVERSION SPELLING" title={`${question.triad.label}（${question.triad.symbol}）`} subtitle="听原位或转位，按实际低到高填写三个音名。" />
...
function ChordToneExercise({ question, feedback, noteValues, onChange, onPlay, onSubmit }: { question: Extract<PracticeQuestion, { kind: 'chord-tone' }>; feedback: AnswerFeedback | null; noteValues: string[]; onChange: (values: string[]) => void; onPlay: () => void; onSubmit: () => void }) {
  ...
  return (
    <>
      <QuestionHeading eyebrow="CHORD MEMBER" title={`${question.triad.label}（${question.triad.symbol}）`} subtitle={<>{'这个和弦的 '}<strong className="question-target">{target}</strong>{' 是什么？'}</>} />
...
function ProgressionExercise({ question, feedback, noteValues, degreeValues, qualities, activeSlot, activeStep, onActiveSlotChange, onNoteChange, onQualityChange, onDegreeChange, onPlay, onSubmitNotes, onSubmitDegrees }: { question: ProgressionQuestion; feedback: AnswerFeedback | null; noteValues: string[]; degreeValues: number[]; qualities: TriadQuality[]; activeSlot: number; activeStep: number | null; onActiveSlotChange: (index: number) => void; onNoteChange: (values: string[]) => void; onQualityChange: (qualities: TriadQuality[]) => void; onDegreeChange: (values: number[]) => void; onPlay: () => void; onSubmitNotes: () => void; onSubmitDegrees: () => void }) {
  ...
  const setQuality = (quality: TriadQuality) => {
  ...
  return (
    <>
      ...
      {forward ? <>
        ...
        <div className="quality-row">{(['major', 'minor', 'diminished'] as TriadQuality[]).map((quality) => <button type="button" key={quality} className={qualities[activeSlot] === quality ? 'selected' : ''} disabled={Boolean(feedback)} onClick={() => setQuality(quality)}>{QUALITY_NAMES[quality]}</button>)}</div>
      ...
      {feedback && <p className="progression-answer">正确答案：{forward ? question.triads.map((triad) => triad.symbol).join(' – ') : question.triads.map((triad) => triad.roman).join(' – ')}</p>}
    </>
  )
}
...
function FeedbackPanel({ question, feedback, onReplay, onNext, isLast }: { question: PracticeQuestion; feedback: AnswerFeedback; onReplay: () => void; onNext: () => void; isLast: boolean }) {
  ...
  else explanation = `${question.key.name} 大调中：${question.triads.map((triad) => `${triad.roman}=${triad.symbol}`).join('，')}。`
