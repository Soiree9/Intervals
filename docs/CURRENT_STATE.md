...
未验收/下一步：用户 2026-08-31 决定停止继续叠加动画触发修复，保留 `b697ca1` 所代表的最初手机交互体验，并只把这一 UI/交互范围推送到 GitHub 功能分支；真机反馈中的中途误触发与返回闪动不在本次继续修改。该分支不合并 `codex/furch-audio-preview`，也不提交本地 `furch-Sample/`。
...
`feat/ui-polish` 分支：把当前手机 UI/交互范围独立保存在 GitHub 功能分支，不合并音源试听工作；`codex/furch-audio-preview` 分支：先用本地 Furch 样本在音程模块完成一次小范围试听测试，不做全局音源替换或大范围音频测试；试听结果得到用户确认后再决定压缩、运行时接入与旧古典吉他音源替换。当前首页结构、默认主图、顶部导航与页脚排版已选定并随 V0.9.0 发布；后续只在用户明确要求时重新接入胶片质感或补充细节艺术设计。比波普解构方向以后另开分支试验，不在当前分支提前实现。除此之外，不要从 [`IDEAS.md`](IDEAS.md) 或 AI 推测中自行扩展范围。
