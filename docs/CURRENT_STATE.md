...
第一阶段首页视觉重构已进入 `main`。用户已选定 D 首页结构、B 高反差衬线字标和“纯净清晰”默认主图：白底蓝色 `INTERVALS` 可点击细线箭头或随下滚收起，原始大近景保持透明音叉与清透亮蓝、朱红、荧光黄绿，全白文案和统计直接进入照片底部。真实页面不再显示 A/B 或主图质感评审器；胶片颗粒版文件仍保留，但只有用户以后明确要求时才接回。共享顶部导航已改为轻字重、细分隔与同一浅灰细线体系：音程品牌标保留，记法采用评审 B、错题采用评审 A，古典吉他暂用正向竖直的镂空琴头并与横向钢琴保持接近的视觉面积。宽屏下字标适度收小、主图文案与统计适度放大；中窄屏保持原比例。下方 `2 × 2` 入口继续使用清楚的标准乐理图形，练习页面保持克制并以谱面、题目和作答为中心。具体边界见 [`DECISIONS.md`](DECISIONS.md#d-014-编辑式视觉基线与音乐文化边界)。
