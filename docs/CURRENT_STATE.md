...
在已发布基线上，本地已完成页脚“音源与许可”入口收拢，以及安装卡片说明的展开、收起和响应式布局微调；这组细节尚未上传或发布。本地 `furch-Sample/` 有六根弦、每根弦 0–12 品共 78 个 WAV，原始目录保持未跟踪；先在音程模块做小范围试听测试，再决定处理和替换路径，试听通过前不提交这批约 31.4 MiB 的原始文件。
