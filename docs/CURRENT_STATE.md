...
当前分支：`codex/home-module-editorial-preview`，从 `main` 的首页视觉基线分出，仅用于下方入口的本地评审，尚未合并或发布。
...
本评审分支正在把下方入口改为更安静的图片目录：桌面 `2 × N` 留出白色间距并限制宽屏卡片高度，手机以约 `3 : 2` 的单列卡片连续浏览；新增模块可继续向下排，奇数尾项在桌面改为全宽半高。四张候选图均不内嵌文字，分别用两个音叉、三个音叉、中心与六个周边标记、单个便携练习设备表达音程、和弦、调与离线。主图本身未更换；模块文案已收成左下角的编号、衬线标题、轻字重说明和文字式入口，和弦卡片的高亮红已压暗并改用暖白文字。浏览器标签和 PWA 安装图标也已从旧的五线谱彩色音符改为首页同源的细线音程关系标。当前仍属于本地视觉评审，未合并或发布。具体边界见 [`DECISIONS.md`](DECISIONS.md#d-014-编辑式视觉基线与音乐文化边界)。
