...
- 2026-08-31，桌面首页比例收紧后通过 `npm.cmd run check`：lint、17 个测试文件共 145 项测试、TypeScript/Vite/PWA 生产构建。真实本地浏览器在 2560×1482 下确认首页画布由 2497px 收到 1520px、主图高度由 1664px 收到 855px，字标由 204px 收到 138px，主图标题/入口标题为 52px/46px，二级页标题保持 48px；1440×1000 下首页仍为双列且无横向溢出。390×844 下仍为 46px Hero、34px 卡片标题、`3 : 4` 主图，无横向溢出；各视口错误级控制台输出为 0。
